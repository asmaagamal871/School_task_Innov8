<?php
$result = $_SESSION['result'] ?? null;
$inputFields = $_SESSION['input_fields'] ?? ['teacher_name' => '', 'teacher_subjects'=>[]];
unset($_SESSION['result'], $_SESSION['input_fields']);
?>
<?php if ($result != null && !$result->success): ?>
    <?php if (count($result->errors) > 0): ?>
        <ul style="color: red;">
            <?php foreach ($result->errors as $error): ?>
                <li><?php echo $error; ?></li>
            <?php endforeach; ?>
        </ul>
    <?php else: ?>
        <p><?php echo $result->message; ?></p>
    <?php endif; ?>
<?php endif; ?>
<div class="d-flex justify-content-center align-items-center pt-5">
    <div class="bg-light bg-gradient p-5 rounded shadow">
        <h2><?= $id === 0 ? 'Create new teacher' : 'Edit teacher' ?></h2>
        <form method="POST" action=<?= $id === 0 ? "/school_project/teachers/create" : "/school_project/teachers/edit/" . $teacherData['teacher_id'] ?>>
            <label class="py-2">Teacher name</label><br>

            <input
                class="form-control"
                type="text"
                placeholder="Enter teacher name"
                aria-label="default input example"
                name="teacher_name"

                value=<?= $id === 0 ? $inputFields['teacher_name'] : $teacherData['teacher_name'] ?>>
                
                <label class="py-2">Subjects</label><br>
            <select class="form-select" multiple aria-label="multiple select example" name="teacher_subjects[]">
                <?php foreach ($subjects as $subject): ?>
                <option 
                        value="<?= $subject['subject_id'] ?>"
                        <?php 
                            $selectedSubjects = $id === 0 
                                ? $inputFields['teacher_subjects'] 
                                : $teacherData['subjects']; 
                            echo in_array($subject['subject_id'], $selectedSubjects) ? 'selected' : ''; 
                        ?>
                    >
                        <?= htmlspecialchars($subject['subject_name']) ?>
                    </option>
                <?php endforeach; ?>

            </select>
            <br>
            <button class="btn btn-primary" type="submit">Save</button>
        </form>
    </div>
</div>